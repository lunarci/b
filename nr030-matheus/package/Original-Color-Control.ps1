...
function Assert-OriginalColorApplySettings($Paths) {
    ...
    foreach ($folder in @($Paths.Plugins,$Paths.OldPlugins)) {
        ...
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
            if (Same-Path $folder $Paths.Plugins) { throw 'The existing pinned base NR ASI is required.' }
            continue
        }
...
function Show-OriginalColorStatus($Paths) {
    ...
    if ($null -eq $state) { throw 'Existing add-on ownership record is required.' }
...
function Invoke-OriginalColorControl($Paths,[ValidateSet('Apply','Restore')][string]$Mode,[scriptblock]$BeforeOperation=$null) {
    ...
    $state=Read-AddonState $Paths
    if ($null -eq $state) { throw 'Existing add-on ownership record is required. No files were changed.' }
    Assert-OwnedFiles $Paths $state -AllowModifiedIni
    $trial=Read-OriginalColorTrial $Paths $state
    ...
    if ($Mode -ceq 'Apply') {
        ...
        $newTrial=[pscustomobject]@{SchemaVersion=1;Root=$Paths.Root;BaselineHash=$script:OriginalColorBaselineHash;TargetHash=$targetHash;BackupFolder=$folder;OriginalStateBackup=$originalState;OriginalStateHash=(Get-Hash $originalState);StateCoreHash=(Get-OriginalColorJsonHash $newState);Files=$records}
    ...
    $gate={ param($index)
        ...
        $source=$operations[$index].Source;Assert-NoReparse $source
    ...
    else { Write-Host 'Exact original add-on ASIs and ownership record restored. All INI files, including later user edits, were preserved.' }
}
