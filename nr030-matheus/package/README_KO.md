...
0.2.3은 NR 결과 중 과도하게 밝은 픽셀이 주변으로 번질 수 있는 잔차 합성 순서를 수정합니다. 설치 성공이나 정상 실행 로그만으로 실제 영상 품질이 검증되는 것은 아닙니다.

- **보정량을 먼저 제한한 뒤 주변 픽셀을 섞습니다.** 이전에는 먼저 섞고 제한해, 아주 조금만 섞여야 하는 매우 밝은 NR 픽셀도 주변의 보정 한도를 모두 차지할 수 있었습니다.
- 원본 색과의 일치도도 평균값이 아닌 각각의 저해상도 픽셀을 기준으로 검사합니다.
- 같은 밝기의 정상 HDR 영역, 원본과 같은 NR 결과, 알파 보존은 유지합니다.
- 새 GPU 텍스처나 이전 프레임 누적 필터를 추가하지 않습니다. 기본 NR·모델·FSR·XeFG 실행 파일은 0.2.2와 같습니다.
- 필요한 NR·모델 다운로드, 배율 2.0 설정, 4채널 모션벡터 지원, 실패 시 NR 없이 우회하는 동작은 유지합니다.

이번 수정은 **밝은 이상 픽셀이 주변으로 번지는 수치 문제**를 대상으로 합니다. 기본 NR 자체가 매 프레임 다르게 바꾸는 반사광·피부 표현까지 안정화한 것은 아닙니다. 세부 경계에서 보정이 약해질 수 있으며 실제 게임의 흰색 깨짐·잔상이 모두 해결됐다고 검증하지 않았습니다.

기본 설정: `ScalePercent=85`, `ColourPreservationPercent=100`, `DepthProtection=1`, `EffectPercent=50`.
...
같은 저장 위치·시선·조명에서 1번 적용 상태와 4번 NR 끔 상태를 비교합니다. 팔을 움직일 때의 흰색 깨짐과 피부 반사면 자글거림을 먼저 확인합니다. 맵을 열기 전, 닫은 직후, 10~30초 뒤 FPS와 피부·잔상을 확인하십시오. `ScalePercent=100`은 원래 크기 기본 NR을 실행하는 별도 비교값이며 NR을 끄는 값이 아닙니다. 모든 설정 변경은 게임 재실행 후 적용됩니다.
