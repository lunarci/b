"""Build the logging-only diagnostic companion from the exact 0.2.4 baseline."""
import argparse
import hashlib
import json
...
NAME = 'OptiScaler_Runtime_Diagnostic'
...
def main():
    ...
    for name in ('Setup.ps1', 'Complete-Setup.ps1', 'Motion-Tuning.ps1', 'Opti-Log-Control.ps1'):
        files[name] = (package / name).read_bytes()
    for name in ('Test-Motion-Tuning.ps1', 'Test-Opti-Log-Control.ps1'):
        files['source/' + name] = (package / name).read_bytes()
    files['README_KO.md'] = (component / 'opti-log-diagnostic/README_KO.md').read_bytes()
    files['source/Build-OptiLog-Package.py'] = Path(__file__).read_bytes()
    for path, script, action in (
        ('01_ENABLE_LOG.cmd', 'Opti-Log-Control.ps1', 'Enable'),
        ('02_COLLECT_LOGS.cmd', 'Motion-Tuning.ps1', 'Collect'),
        ('03_RESTORE_LOG_SETTINGS.cmd', 'Opti-Log-Control.ps1', 'Restore'),
    ):
    ...
    provenance = {
        'name': NAME,
        'type': 'logging-only diagnostic companion; not a graphics fix',
    ...
    files['DIAGNOSTIC_PROVENANCE.json'] = json.dumps(provenance, ensure_ascii=False, indent=2).encode('utf-8')
